...
enum class TestType { Fake, Base };

class TestSourceNodeBuilder
{
public:
    TestSourceNodeBuilder(FakeSourceNodeParams params) :
        _fake_source_node_params(params),
        _test_type(TestType::Fake)
    {}

    TestSourceNodeBuilder(DataThreadCreator creator) :
        _data_thread_creator(creator),
        _test_type(TestType::Base)
    {}
...
public:
    ...
    void setupForNonSource()
    {
        SourceNode* sn_temp = source_node_builder.build();
    ...
    T* CreateDataThread()
    {
        SourceNode* sn = source_node_builder.build();
        T* ptr = new T(sn);
        sn->setProcessorType(Plugin::Processor::Type::SOURCE);
        sn->setHeadlessMode(true);

        int node_id = next_processor_id_++;
        source_node_id = node_id;
        sn->setNodeId(node_id);
        processor_graph->addNode(
            std::move(std::unique_ptr<AudioProcessor>(sn)),
            juce::AudioProcessorGraph::NodeID(node_id));
...
private:
    ...
    TestSourceNodeBuilder source_node_builder;
