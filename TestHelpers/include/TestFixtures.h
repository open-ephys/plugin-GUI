...
{
public:
    TestSourceNodeBuilder (FakeSourceNodeParams params) : _fake_source_node_params (params),
                                                          _source_node_type (TestSourceNodeType::Fake) {}
    TestSourceNodeBuilder (DataThreadCreator creator) : _data_thread_creator (creator),
                                                        _source_node_type (TestSourceNodeType::Base) {}
...
public:
        ...
        TTLEvent* maybe_ttl_event = nullptr)
    {
        ...
        for (const auto* datastream : data_streams)
        {
            ...
            {
                size_t ttl_size = maybe_ttl_event->getChannelInfo()->getDataSize() + maybe_ttl_event->getChannelInfo()->getTotalEventMetadataSize() + EVENT_BASE_SIZE;
...
{
public:
    DataThreadTester (TestSourceNodeBuilder source_node_builder) : ProcessorTester (source_node_builder)
    {
    }
