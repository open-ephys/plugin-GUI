...
bool TrialCircularBuffer::parseMessage(StringTS msg)
{
	...
	else if (command == "clean_psth_database")
	{
		const ScopedLock myScopedLock(psthMutex);
		electrodesPSTH.clear();
		redrawNeeded = true;
	}
	else if (command == "newelectrode")
	{
		int electrodeID = input[1].getIntValue();
		String name = input[2]; 
		int numChannels = input[3].getIntValue();
		int *channels = new int[numChannels];
		for (int k = 0; k < numChannels;k++ )
			channels[k] = input[4+k].getIntValue();

		addNewElectrode(electrodeID, name, numChannels,channels);
		delete channels;
	...
	else if (command == "newunit")
	{
		...
		int colR = input[3].getIntValue();
		int colG = input[4].getIntValue();
		int colB = input[5].getIntValue();

		addNewUnit(electrodeID, unitID, colR, colG, colB);
		redrawNeeded = true;
	}	else if (command == "removeunit")
	{
		int electrodeID = input[1].getIntValue();
		int unitID = input[2].getIntValue();
		removeUnit(electrodeID, unitID);
		redrawNeeded = true;
	}
	else if (command == "removeallunits")
	{
		int electrodeID = input[1].getIntValue();
		removeAllUnits(electrodeID);
		redrawNeeded = true;
	}
	else if (command == "removeelectrode")
	{
		int electrodeID = input[1].getIntValue();
		removeElectrode(electrodeID);
		redrawNeeded = true;
	}
	else if (command == "changeelectrodechannel") {
		// TODO
	}
...
{
	const ScopedLock myScopedLock(psthMutex);
	ElectrodePSTH e(electrodeID, name);

	for (int k = 0; k < numChannels; k++)
	{
		int channelID = channelIDs[k];
