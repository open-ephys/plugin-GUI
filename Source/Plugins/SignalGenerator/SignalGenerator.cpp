...
float SignalGenerator::generateSpikeSample(double amp, double phase, double noise)
{
    ...
    const int shift = -9500;//1000 + 32768;
    const int gain  = 8000;

    // Generate random number between -.1 and .1
    const double r = ((rand() % 201) - 100) / 1000.0;
    // Shrink the range of r based upon the value of noise
    noise = r * noise / (double_Pi * 2);

    // bind between 0 and N_SAMP-1, too tired to figure out the proper math
    const int sampIdx = (int)(phase / (2 * double_Pi) * (N_WAVEFORM_SAMPLES-1));
    //sampIdx = sampIdx + 8;

    // Right now only sample from the 3rd waveform.
    // I need to figure out a way to only sample from a single spike until the phase wraps
    const float baseline = shift + gain *  SPIKE_WAVEFORMS[spikeIdx][1] ;

    const float sample = shift + gain * (SPIKE_WAVEFORMS[spikeIdx][sampIdx] + noise);  // * pow( WAVEFORM_SCALE[sampIdx], amp / 200.0 ) ) ;
    const float dV = sample  - baseline;
    dV = dV * (1 + amp / 250);
    sample = baseline + dV;
