...
FileReader::FileReader()
    : GenericProcessor("File Reader"), Thread("filereader_Async_Reader"),
      totalSamplesAcquired(0), currentSampleRate(0), currentNumChannels(0),
      currentSample(0), currentNumTotalSamples(0), currentNumScrubbedSamples(0),
      startSample(0), stopSample(0), loopCount(0), bufferCacheWindow(0),
      m_shouldFillBackBuffer(false), m_bufferSize(1024), m_sysSampleRate(44100),
      playbackActive(true), gotNewFile(true), loopPlayback(true) {

  /* Load any plugin file sources */
  const int numFileSources =
      AccessClass::getPluginManager()->getNumFileSources();

  LOGD("Found ", numFileSources, " File Source plugins.");

  for (int i = 0; i < numFileSources; ++i) {
    Plugin::FileSourceInfo info =
        AccessClass::getPluginManager()->getFileSourceInfo(i);
  ...
  for (int i = 0; i < numBuiltInFileSources; ++i) {
    ...
    const int numExtensions = extensions.size();
    for (int j = 0; j < numExtensions; ++j) {
      supportedExtensions.set(extensions[j].toLowerCase(),
                              i + numFileSources + 1);
    }
  }

  /* Create a File Reader device */
  DeviceInfo::Settings settings{"File Reader", "description", "identifier",
                                "00000x003", "Open Ephys"};
...
#ifdef __APPLE__
  File defaultFile = executable.getChildFile("Contents/Resources/resources")
                         .getChildFile("structure.oebin");
#else
  File defaultFile = executable.getParentDirectory()
...
void FileReader::togglePlayback() { playbackActive = !playbackActive; }

bool FileReader::playbackIsActive() { return playbackActive; }

int64 FileReader::getCurrentNumTotalSamples() { return currentNumTotalSamples; }
...
bool FileReader::setFile(String fullpath) {
  ...
  if (isExtensionSupported) {
    const int index = supportedExtensions[ext] - 1;
    const int numPluginFileSources =
        AccessClass::getPluginManager()->getNumFileSources();

    if (index < numPluginFileSources) {
      Plugin::FileSourceInfo sourceInfo =
          AccessClass::getPluginManager()->getFileSourceInfo(index);
...
void FileReader::setActiveRecording(int index) {
  ...
  static_cast<FileReaderEditor *>(getEditor())
      ->setTotalTime(samplesToMilliseconds(currentNumTotalSamples));
  input->seekTo(startSample);

  gotNewFile = true;
}

int64 FileReader::getCurrentSample() { return currentSample; }
...
void FileReader::updateSettings() {
  ...
  if (gotNewFile) {
    ...
    /* Only use the original stream name (FileReader-100.example_data ->
     * example_data) */
    StringArray tokens;
    ...
    DataStream::Settings streamSettings{
        ...
        streamName, "A description of the File Reader Stream", "identifier",
    ...
    for (int i = 0; i < currentNumChannels; i++) {
      String chan_name = channelInfo[i].name;
      if (chan_name.contains("AUX")) {
    ...
    EventChannel::Settings eventSettings{
        EventChannel::Type::TTL, "All TTL events",
        "All TTL events loaded for the current input data source",
        "filereader.events", dataStreams.getLast()};

    // FIXME: Should add an event channel for each event channel detected in the
    // current file source
  ...
  if (m_bufferSize == 0)
    m_bufferSize = 1024;
  m_samplesPerBuffer.set(m_bufferSize *
                         (getDefaultSampleRate() / m_sysSampleRate));
...
int FileReader::getPlaybackStart() { return startSample; }

int FileReader::getPlaybackStop() { return stopSample; }
...
String FileReader::handleConfigMessage(String msg) {
  ...
  if (tokens[0] == "file")
    ...
    static_cast<FileReaderEditor *>(getEditor())
        ->setRecording(std::stoi(tokens[1].toStdString()));
  else if (tokens[0] == "start")
    static_cast<FileReaderEditor *>(getEditor())
        ->setPlaybackStartTime(std::stoi(tokens[1].toStdString()));
...
void FileReader::process(AudioBuffer<float> &buffer) {
  ...
  int samplesNeededPerBuffer = int(float(buffer.getNumSamples()) *
                                   (getDefaultSampleRate() / m_sysSampleRate));

  if (!playbackActive &&
      totalSamplesAcquired + samplesNeededPerBuffer > stopSample) {
  ...
  // if cache window id == 0, we need to read and cache BUFFER_WINDOW_CACHE_SIZE
  // more buffer windows
  if (bufferCacheWindow == 0) {
    switchBuffer();
  }

  // std::cout << "Reading " << samplesNeededPerBuffer << " samples. " <<
  // std::endl;

  for (int i = 0; i < currentNumChannels; ++i) {
    // offset readBuffer index by current cache window count * buffer window
    // size * num channels
    input->processChannelData(
        *readBuffer +
            (samplesNeededPerBuffer * currentNumChannels * bufferCacheWindow),
        buffer.getWritePointer(i, 0), i, samplesNeededPerBuffer);
  }

  setTimestampAndSamples(totalSamplesAcquired, -1.0, samplesNeededPerBuffer,
                         dataStreams[0]->getStreamId()); // TODO: Look at this
...
void FileReader::addEventsInRange(int64 start, int64 stop) {
  ...
  for (int i = 0; i < events.channels.size(); i++) {
    ...
    juce::int64 absoluteCurrentTimestamp =
        events.timestamps[i] + loopCount * (stopSample - startSample);
    if (events.text.size() && !events.text[i].isEmpty()) {
      String msg = events.text[i];
      LOGD("Broadcasting message: ", msg,
           " at timestamp: ", absoluteCurrentTimestamp,
           " channel: ", events.channels[i]);
      broadcastMessage(msg);
    } else {
      uint8 ttlBit = events.channels[i];
      bool state = events.channelStates[i] > 0;
      TTLEventPtr event = TTLEvent::createTTLEvent(
          eventChannels[0], events.timestamps[i], ttlBit, state);
...
void FileReader::setParameter(int parameterIndex, float newValue) {
  ...
  case 1:
    ...
    static_cast<FileReaderEditor *>(getEditor())
        ->setCurrentTime(samplesToMilliseconds(currentSample));
    break;
...
HeapBlock<int16> *FileReader::getFrontBuffer() { return readBuffer; }
...
void FileReader::readAndFillBufferCache(HeapBlock<int16> &cacheBuffer) {
  ...
  while (samplesRead < samplesNeeded) {
    ...
    if ((currentSample + samplesToRead) > stopSample) {
      ...
      if (samplesToRead > 0)
        input->readData(cacheBuffer + samplesRead * currentNumChannels,
                        samplesToRead);
...
int FileReader::getNumBuiltInFileSources() const { return 1; }
