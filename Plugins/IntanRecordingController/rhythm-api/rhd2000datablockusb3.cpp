...
// from a Rhythm USB3 FPGA interface controlling up to 16 RHD2000 chips.

// Constructor.  Allocates memory for data block.
Rhd2000DataBlockUsb3::Rhd2000DataBlockUsb3(int numDataStreams)
{
...
Rhd2000DataBlockUsb3::~Rhd2000DataBlockUsb3()
{
...
// Copy constructor
Rhd2000DataBlockUsb3::Rhd2000DataBlockUsb3(const Rhd2000DataBlockUsb3 &obj)
{
...
void Rhd2000DataBlockUsb3::allocateIntArray1D(vector<int> &array1D, int xSize)
{
    array1D.resize(xSize);
}

// Allocates memory for a 1-D array of unsigned integers.
void Rhd2000DataBlockUsb3::allocateUIntArray1D(vector<unsigned int> &array1D, int xSize)
{
    array1D.resize(xSize);
}

// Allocates memory for a 2-D array of integers.
void Rhd2000DataBlockUsb3::allocateIntArray2D(vector<vector<int> > & array2D, int xSize, int ySize)
{
    int i;
...
void Rhd2000DataBlockUsb3::allocateIntArray3D(vector<vector<vector<int> > > &array3D, int xSize, int ySize, int zSize)
{
    int i, j;
...
unsigned int Rhd2000DataBlockUsb3::getSamplesPerDataBlock()
{
...
unsigned int Rhd2000DataBlockUsb3::calculateDataBlockSizeInWords(int numDataStreams, int nSamples)
{
	unsigned int samps = nSamples <= 0 ? SAMPLES_PER_DATA_BLOCK : nSamples;
    return samps * (4 + 2 + (numDataStreams * (CHANNELS_PER_STREAM + 3)) + (numDataStreams % 4) + 8 + 2);
...
bool Rhd2000DataBlockUsb3::checkUsbHeader(unsigned char usbBuffer[], int index)
{
...
unsigned int Rhd2000DataBlockUsb3::convertUsbTimeStamp(unsigned char usbBuffer[], int index)
{
...
int Rhd2000DataBlockUsb3::convertUsbWord(unsigned char usbBuffer[], int index)
{
...
// Print the contents of RHD2000 registers from a selected USB data stream (0-31)
// to the console.
void Rhd2000DataBlockUsb3::print(int stream) const
{
...
void Rhd2000DataBlockUsb3::writeWordLittleEndian(ofstream &outputStream, int dataWord) const
{
...
void Rhd2000DataBlockUsb3::write(ofstream &saveOut, int numDataStreams) const
{
