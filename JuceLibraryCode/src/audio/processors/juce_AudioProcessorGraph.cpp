...
private:
    ...
    int getBufferContaining (const uint32 nodeId, const int outputChannel) const throw()
    {
...
public:
    ...
    bool isAnInputTo (const uint32 possibleInputId,
                      const uint32 possibleDestinationId) const throw()
    {
...
private:
    ...
    struct Entry
    {
        explicit Entry (const uint32 destNodeId_) throw() : destNodeId (destNodeId_) {}
    ...
    bool isAnInputToRecursive (const uint32 possibleInputId,
                               ...
                               int recursionCheck) const throw()
    {
        int index;
    ...
    Entry* findEntry (const uint32 destNodeId, int& insertIndex) const throw()
    {
        Entry* result = 0;
...
struct ConnectionSorter
{
    static int compareElements (const AudioProcessorGraph::Connection* const first,
                                const AudioProcessorGraph::Connection* const second) throw()
    {
...
AudioProcessorGraph::Connection::Connection (const uint32 sourceNodeId_, const int sourceChannelIndex_,
                                             const uint32 destNodeId_, const int destChannelIndex_) throw()
...
//==============================================================================
AudioProcessorGraph::Node::Node (const uint32 nodeId_, AudioProcessor* const processor_) throw()
    : nodeId (nodeId_),
